...
Vercel HTML에 측정 ID `G-R553VVDCZ4`의 Google tag를 한 번 삽입합니다. Sites 원본 주소와 localhost에는 삽입하지 않습니다. 앱의 동일 출처 `/api/meeting` POST 응답이 성공하고 room을 반환한 경우에만 다음 맞춤 이벤트를 전송합니다. 원래 fetch 응답은 그대로 앱에 전달하며 분석 오류는 앱 동작에 영향을 주지 않습니다.

| 이벤트 | 기록 시점 |
| --- | --- |
| `meeting_created` | 모임 생성 성공 |
| `meeting_joined` | 초대 코드로 입장 성공 (기존 멤버 재입장 포함) |
| `availability_submitted` | 가능 일정 제출 성공 (수정 제출 포함) |
| `meeting_date_confirmed` | 방장이 날짜 선택 완료 (참석 재확인이 필요한 수동 선택 포함) |
| `region_vote_submitted` | 지역 투표 제출 성공 (수정·재투표 포함) |
| `meeting_location_confirmed` | 마지막 지역 투표 또는 무작위 선택으로 최종 지역 확정 |
| `place_link_added` | 음식점·카페 링크 추가 성공 |

이벤트에는 사용자 정의 매개변수를 보내지 않습니다. 이벤트 횟수는 고유 모임 수나 고유 인원 수가 아닙니다. 새로고침, 정기 조회, 로그인 및 실패한 제출은 위 이벤트를 발생시키지 않습니다. 서버 응답을 브라우저에서 관찰하는 방식이므로 추적 차단·탭 종료 등에 따른 누락이 가능하며 DB 통계의 대체 수단이 아닙니다. 멤버 강퇴·탈퇴로 자동 확정되는 지역은 이 이벤트 범위에서 제외합니다.

GA4 관리 화면에 같은 이름의 파생 이벤트를 추가로 만들 필요는 없습니다. 배포 후 실제 동작을 하면 실시간 보고서에 이벤트가 수신됩니다. 일반 이벤트 보고서 반영에는 시간이 걸릴 수 있습니다.
